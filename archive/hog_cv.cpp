...
int strategies[20][101][101];

int s1,s2;

int strategy_s1(int player_score, int opponent_score){
  return strategies[s1][player_score][opponent_score];
}

int strategy_s2(int player_score, int opponent_score){ 
  return strategies[s2][player_score][opponent_score];
}
...
void cross_validate(){
  ...
  for(int k = 1; k <= 19; ++k){
    ...
    printf("\n");
    for(int i = 0; i < _S1; ++i){
      for(int j = 0; j < _S2; ++j) strategies[k-1][i][j] = strategy[i][j];
    }
  }
  for(int i = 0; i < 19; ++i){
    for(int j = i+1; j < 19; ++j){
        ...
        float wr = average_win_rate(strategy_s1, strategy_s2, 1000000);
        printf("winrate: %f\n", wr);
        if(wr > 0.5f){
          points[i]++;
        }else{
  ...
  for(int i = 0; i < 19; ++i) printf("%d: %d ", i+1, points[i]);
  printf("\n");
}
