...
void train_initial(){
  ...
  for(int i = 0; i < epochs; ++i){
    float winrates[5];
    winrates[0] = average_win_rate(rl, always_roll_4,50000);
    winrates[1] = average_win_rate(rl, always_roll_random,50000);
    winrates[2] = average_win_rate(rl, rl_no_effect,50000);
    winrates[3] = average_win_rate(rl, swap_and_5,50000);
    winrates[4] = average_win_rate(rl, final_strategy,50000);
    ...
    printf("epoch: %4d type: fin  epsilon: %lf winrate: %f\n", i + 1, epsilon, winrates[4]);
